...
- **Chapter 4: Pull & JIT Systems (Case Study 1):** Airport terminal expansion scenario (12 days) teaching Pull Systems, JIT delivery, Safety Buffers, and the Bullwhip Effect via the PullSystemDashboard. Features a dedicated HUD bar (Day/Funds/PDI/Hoists/Rework/End Day), guided tutorial (steps 20-21: Pull Board, JIT Scheduler), 8 narrative decisions spread across days 2-11 (triggered after dialogue completion), daily narrative dialogues from CASE_1_SCHEDULE, and a 5-question reflection quiz before chapter results. `receiveDeliveries()` processes scheduled material arrivals at each day start.
