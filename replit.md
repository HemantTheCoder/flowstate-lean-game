...
- **Chapter 4: Pull & JIT Systems (Case Study 1):** Airport terminal expansion scenario (5 days) teaching Pull Systems, JIT delivery, Safety Buffers, and the Bullwhip Effect via the PullSystemDashboard. Features a dedicated HUD bar (Day/Funds/PDI/Hoists/Rework/End Day), guided 5-step spotlight tutorial (steps 20-24: Pull System intro, WIP Limits, JIT Scheduler, Safety Buffers, End Day), 3 narrative decisions on Days 2-4 (VIP Demand Spike, Bullwhip Warning, Truck Breakdown), daily narrative dialogues from CASE_1_SCHEDULE, and a 5-question reflection quiz before chapter results. `receiveDeliveries()` processes scheduled material arrivals at each day start. Starting inventory provided (timber: 30, pipes: 20, electrical: 20). Day-start flags are chapter-scoped (`ch${chapter}_day_${day}_started`) to prevent cross-chapter collisions.
